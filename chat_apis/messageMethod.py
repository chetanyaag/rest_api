...
def verifyGST(gstNumber):
    ...
    if bool(boolForGST) or bool(bool_hsn) or bool(nba) or bool(fillingFreq) or bool(returns):
        ...
        if returns:
            returnString = returnString + "Financial Year : " + returns[0]['fy'] + "\n"
            returnString = returnString + "Date Of Filling : " + returns[0]['dof'] + "\n"
            returnString = returnString + "Tax Period : " + returns[0]["taxp"] + "\n"
...
def create_employee_status(filtered_rows):

    employe_name = list(filtered_rows['EMPLOYEE_NAME'])[0]
    month = list(filtered_rows['Month'])[0]
    reciving_date = list(filtered_rows['Rec_Date'])[0]
    claim_amount = list(filtered_rows['Claim_amount'])[0]
    status =  list(filtered_rows['Status'])[0]

    return_string = ""
    return_string =return_string+ "--------------------------------------------------\n                   DETAILS                      \n--------------------------------------------------\n"

    return_string = return_string + " Name                 :  " + employe_name  + "\n Month                : " + str(month.strftime("%d/%m/%y"))
    return_string = return_string + "\n Receiving Date :  " + str(reciving_date.strftime("%d/%m/%y"))  
    return_string = return_string + "\n Claim amount  :  " + str(claim_amount)+ "\n Deduction         :  " + str(status)
    # return_string = return_string +"\n Reasons            :  \n" + reason +"\n"
...
def returnStatus(emp_code, month, year):
    ...
    try:
        ...
        if (str(month) == str(date.month)) and (str(year) ==str(date.year)[-2:]):
            return create_employee_status(filtered_rows)
        else:
...
def conversation(number , message_text):
    ...
    if ("gst" in message_text) or ("GST" in message_text) :
        gstNumber = (message_text.split(" "))
        if (len(gstNumber) ==2) and (gstNumber[1].strip()).isalnum():
            return verifyGST(gstNumber[1])
        else:
            return "GST Number is given in wrong format"
    ...
    if message_text in ['cancel', '/cancel', 'stop','restart', 'Cancel', 'Stop', 'Restart']:
        ...
        if number in ["918445741113", "919027782731"] :
            return "Chat cleaned, \nYou can re initiate the chat."
        return "Bye! I hope we can talk again some day."


    if number in ["918445741113","919027782731"]:
        return admin_conversation(number, message_text, chat_user)
...
def admin_conversation(number, message, chat_user):
    ...
    try:
        ...
        if chat_state == "1":
            #check message
            employee = employee_table.find_one({"employee_code": str(message.strip())})
            if employee:
                chat_user_table.update_one({'user_number':number}, {"$set":{"chat_state":"2", "employee_code":employee["employee_code"]}})
                return "Please enter the number of the month and year in mm/yy Format.\nExample: 03/17"
            else:
                return "Sorry, This Employee does not exits in my database"

            
        if chat_state == "2":
            month_status = check_month_year(message)
